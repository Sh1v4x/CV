  ...
  play(keyframeOptions: KeyframeAnimationOptions, flipOptions?: FlipOptions) {
    this.elements.forEach((pair) => {
      ...
      if (endPos.width && endPos.height) {
        const deltaX =
          !flipOptions || flipOptions.enableX ? startPos.x - endPos.x : 0;
        const deltaY =
          !flipOptions || flipOptions.enableY ? startPos.y - endPos.y : 0;
        const scaleW =
          !flipOptions || flipOptions.enableWidth
            ? startPos.width / endPos.width
            : 1;
        const scaleH =
          !flipOptions || flipOptions.enableHeight
            ? startPos.height / endPos.height
            : 1;
