...
export class Header extends HTMLElement {
	...
	private initWriter(): void {
		...
		const writerUserName = () => this.addTimeouts(Writer.write(this.h1, '<I\'m Maxime/>', writerH1Options, () => this.addTimeouts([setTimeout(writerImMaxime, 1000)])))
		const writerImMaxime = () => this.addTimeouts(Writer.write(this.h1, 'I\'m Maxime', writerH1Options, writerWelcome))
		...
		const writerMaxime = () => this.addTimeouts(Writer.write(this.h1, 'Maxime', writerH1Options, writerMaximeBommeLasnier))
		const writerMaximeBommeLasnier = () => this.addTimeouts(Writer.write(this.h1, 'Maxime Bomme--Lasnier', writerH1Options, () => {
			this.addTimeouts([setTimeout(writerClearH1, 15000)])
			this.addTimeouts([setTimeout(writerClearH2, 15000)])
		}))
		const writerImDev = () => this.addTimeouts(Writer.write(this.h2, 'Web developer', writerH2Options))
