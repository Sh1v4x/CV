...
export class Header extends HTMLElement {
  ...
  private initWriter(): void {
    ...
    const writerHelloWorld = () =>
      this.addTimeouts(
        Writer.write(this.h1, "Hello World !", writerH1Options, () =>
          this.addTimeouts([setTimeout(writerHTMLMark, 1000)])
        )
      );
    const writerHTMLMark = () =>
      this.addTimeouts(
        Writer.write(this.h1, "</>", writerH1Options, writerUserName)
      );
    const writerUserName = () =>
      this.addTimeouts(
        Writer.write(this.h1, "<I'm Maxime/>", writerH1Options, () =>
          this.addTimeouts([setTimeout(writerImMaxime, 1000)])
        )
      );
    const writerImMaxime = () =>
      this.addTimeouts(
        Writer.write(this.h1, "I'm Maxime", writerH1Options, writerWelcome)
      );
    const writerWelcome = () =>
      this.addTimeouts(
        Writer.write(this.h2, "Welcome to my website", writerH2Options, () => {
          this.addTimeouts([setTimeout(writerMaxime, 5000)]);
    ...
    const writerMaxime = () =>
      this.addTimeouts(
        Writer.write(
          ...
          "Maxime",
          writerH1Options,
          writerMaximeBommeLasnier
        )
      );
    const writerMaximeBommeLasnier = () =>
      this.addTimeouts(
        Writer.write(this.h1, "Maxime Bomme--Lasnier", writerH1Options, () => {
          this.addTimeouts([setTimeout(writerClearH1, 15000)]);
          this.addTimeouts([setTimeout(writerClearH2, 15000)]);
        })
      );
    const writerImDev = () =>
      this.addTimeouts(Writer.write(this.h2, "Web developer", writerH2Options));
    const writerClearH1 = () =>
      this.addTimeouts(
        Writer.write(this.h1, "", writerH1Options, writerHelloWorld)
      );
    const writerClearH2 = () =>
      this.addTimeouts(Writer.write(this.h2, "", writerH2Options));
  ...
  private onMouseMove(event: MouseEvent): void {
    const mouseX: number =
      parseInt(document.body.style.getPropertyValue("--mouse-x")) | 0;
    const mouseY: number =
      parseInt(document.body.style.getPropertyValue("--mouse-y")) | 0;

    const x = Math.min(Math.max(-1, (mouseX / window.innerWidth - 0.5) * 2), 1);
    const y = Math.min(
      Math.max(-1, (mouseY / window.innerHeight - 0.5) * 2),
      1
    );
